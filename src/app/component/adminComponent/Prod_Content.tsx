import { fetchProductAPI, fetchCategoryAPI, createNewProductAPI, updateProductAPI, deleteProductAPI } from '@/app/api/index'
...
export const Prod_Content = ( {isDisable}: {isDisable: boolean} ) => {
  ...
  const [editingProduct, setEditingProduct] = useState<Product | null>(null) // Lưu danh mục đang chỉnh sửa
  ...
  const handleCategoryInputChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const productData = {
      ...
      price: newProduct.price || 0, // Chuyển price sang số
      quantity: newProduct.quantity || 0 // Chuyển quantity sang số
    }
    ...
    try{
      ...
      let errorMessage: string[] = ['Không thể thêm sản phẩm']
  ...
  const handleDelete = async (productId: string) => {
    if (window.confirm('Bạn có chắc muốn xóa sản phẩm này?')) {
      ...
      try {
        ...
        let errorMessage: string[] = ['Không thể xóa sản phẩm']
        if (err && typeof err === 'object' && 'response' in err) {
          const axiosErr = err as { response?: { data?: { message?: string } } }
          if (axiosErr.response?.data?.message) {
            errorMessage = axiosErr.response.data.message.split("-")
